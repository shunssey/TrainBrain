...
class Button
  def initialize(x, y, string = "", font_size = 36, w = 40, h = 100, bk_color = [200, 200, 200], gr_color = [150,150,150],cg_color = [0,0,0],mz_color=[0,0,0])
    @x ,@y = x, y
    @w ,@h = w, h
    @color = bk_color
    @cg_color = cg_color
    ...
    @string = string
    @font = Font.new(font_size,"MSゴシック")
    #@font = Font.new(font_size,"HG正楷書体-PRO")
    draw_string
    @click = 0
  end
  attr_accessor :x, :y, :string, :image, :font, :bk_color ,:gr_color,:cg_color,:mz_color
  ...
  def draw_string
    ...
    @image.drawFont(@w/2-size/2, @h/2-@font.size/2, @string, @font,@mz_color)
  end
