require 'dxruby'
#Šî–{ImageƒNƒ‰ƒX+ƒLƒƒƒ“ƒoƒX‚Á‚ÄŠ´‚¶
class Images
  attr_accessor :color, :w, :h,:x,:y,:log_flag,:string,:font_size
  def initialize(x,y,color=[255,255,255],w=50,h=50,string="",font_size=32,font_sty="‚l‚r ‚oƒSƒVƒbƒN",log_flag=0)
    @x,@y=x,y
    @w,@h=w,h
    @color=color
  ...
  def point_string2(x,y,string="",font_size=30,font_sty="‚l‚r ‚oƒSƒVƒbƒN",color=[255,255,255])
    @image.drawFont(x, y, string, @font,color)
  end
  ...
  def pen_active(color=[0,0,0],size=3)
    ...
    if Input.mouseDown?(M_LBUTTON)#’Êí‚Í‚±‚¿‚ç‚ğg—p
      if (mx - @x)/@w == 0 and (my - @y)/@h == 0
        $file.puts ",,#{S_Timer.get},,#{$pen_size[$pen_count]},#{$pen_count}"
  ...
  def pen_active2(color,size,x_num ,y_num ,math_w ,math_h,aa,bb)#¡–Ú‚ ‚½‚è”»’è•t‚«
    mx = Input.mousePosX
    my = Input.mousePosY
    if @log_flag==1
      if Input.mousePush?(M_LBUTTON)
...
=end
  ...
  def log
    return @pen_array if @log_flag==1#ƒGƒ‰[–h~
  end
...
class Array_Images
  def initialize(x,y,filename,x_count,y_count,n)
    ...
    @n=n
    @image=Image.loadToArray(filename,x_count,y_count )#”z—ñ‚È‚ñ‚¾‚æ
