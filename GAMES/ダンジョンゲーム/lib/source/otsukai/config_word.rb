...
class Init_Scene  < Scene::Base
  ...
  #タイトル画面の諸々の定義　↓ここから↓
  def config_word_init
    if $word_count <= 3
      for i in 1..$word_count
        #$word[i] = $word[i].shuffle
        $word_teiji[i] = Fonts.new(i*150+100, 100, "#{$word[i][i]}", 24, Black)
      end
    elsif $word_count >= 4 && $word_count <= 6
      for i in 1..3
        #p $word[i] = $word[i].shuffle
        $word_teiji[i] = Fonts.new(i*150+100, 100, "#{$word[i-1][i]}", 24, Black)
      end
      
      for i in 4..$word_count #超過分の単語
        $word_teiji[i] = Fonts.new(i*150-350, 150, "#{$word[i-1][i]}", 24, Black)
      end
    elsif  $word_count >= 7
      for i in 1..3
        $word_teiji[i] = Fonts.new(i*150+100, 100, "#{$word[i-1][i]}", 24, Black)
      ...
      for i in 7..$word_count #超過分の単語
        $word_teiji[i] = Fonts.new(i*150-800, 200, "#{$word[i-6][i-1]}", 24, Black)
      end
    end 
  end
  
  def config_word_update
    
  end
  
  def config_word_render
