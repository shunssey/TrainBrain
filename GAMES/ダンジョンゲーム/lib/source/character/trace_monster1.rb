...
class Init_Scene  < Scene::Base
  ...
  def trace_monster_init
    ...
    $trace_monster_move_count2 = 100
    $monster_direct = 0
  end #init
  
  def trace_monster_update
    $trace_monster_move_count += 1
    # 
    if $trace_monster_move_count > $trace_monster_move_count2
      if $player.x != $monster.x #横の座標に差があるとき
        if $player.x <= $monster.x && $maze["#{$monster.y/30}".to_i]["#{$monster.x/30}".to_i-1] != 1 #左に進む
          $monster = Array_Images.new($monster.x, $monster.y,"./image/character/monster1.png",3,4,3)
          $monster.x -= 30
          $trace_monster_move_count = 0 
        elsif $player.x >= $monster.x && $maze["#{$monster.y/30}".to_i]["#{$monster.x/30}".to_i+1] != 1 #右に進む
          $monster = Array_Images.new($monster.x, $monster.y,"./image/character/monster1.png",3,4,6)
          $monster.x += 30
          $trace_monster_move_count = 0
        end
      end
          
      if $player.y != $monster.y #縦の座標に差があるとき
        if $player.y <= $monster.y && $maze["#{$monster.y/30}".to_i-1]["#{$monster.x/30}".to_i] != 1 #上に進む
          $monster = Array_Images.new($monster.x, $monster.y,"./image/character/monster1.png",3,4,9)
          $monster.y -= 30
          $trace_monster_move_count = 0
        elsif $player.y >= $monster.y && $maze["#{$monster.y/30}".to_i+1]["#{$monster.x/30}".to_i] != 1 #下に進む
          $monster = Array_Images.new($monster.x, $monster.y,"./image/character/monster1.png",3,4,0)
          $monster.y += 30
          $trace_monster_move_count = 0
        end
      end
