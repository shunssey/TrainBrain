...
class Baloon
  def initialize(x=0,y=0,font=nil,num=nil,geki=0,after=0)
    @image = Image.load("image/baloon.png", 0, 0)# if geki == 0
    @image_after = Image.load("image/baloon_after.png", 0, 0) #if geki == 0
  ...
  def color
  @image = Image.load("image/baloon_after.png", 0, 0)
  @image.drawFont(@w/2 - @font_size.getWidth(@font)/2,15,"#{@font}",@font_size,Black)
  end
